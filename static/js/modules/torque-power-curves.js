...
function renderGearGraph() {
    try {
        // Get data from window object
        const data = window.gearCalculationData;
        if (!data) {
            console.error("No gear calculation data available");
            return;
        }
        
        const container = document.getElementById('gearGraph');
        if (!container) {
            console.error("Gear graph container not found");
            return;
        }
        ...
        // Rest of your graph rendering code...
        // (Existing code from the template is quite comprehensive)
